...
//----create by me
-(void)layoutIfNeededWithDuriation:(NSTimeInterval)duriation
{
    [UIView animateWithDuration:duriation animations:^{
        [self layoutIfNeeded];
    }];
}
-(void)removeAllConstranins
{
    [self removeConstraints:self.constraints];
}

-(NSLayoutConstraint*)constrainWidthWithMutiplier:(CGFloat)multiplier
{
    return [self constrainWidthBySuperview:self.superview multiplier:multiplier];
}
-(NSLayoutConstraint *)constrainWidthBySuperview:(UIView*)superview multiplier:(CGFloat)multiplier
{
    NSLayoutConstraint *constraint = [NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:superview attribute:NSLayoutAttributeWidth multiplier:multiplier constant:0];
    [superview addConstraint:constraint];
    return constraint;
}

-(NSLayoutConstraint*)constrainHeightWithMutiplier:(CGFloat)multiplier
{
    return [self constrainHeightBySuperview:self.superview multiplier:multiplier];
}
-(NSLayoutConstraint *)constrainHeightBySuperview:(UIView*)superview multiplier:(CGFloat)multiplier
{
    NSLayoutConstraint *constraint = [NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:superview attribute:NSLayoutAttributeHeight multiplier:multiplier constant:0];
    [superview addConstraint:constraint];
    return constraint;
}
